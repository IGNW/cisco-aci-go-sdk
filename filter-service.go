...
func (fs FilterService) New(name string, description string) *Filter {
	resourceName := fmt.Sprintf("@TODO-%s", name)

	b := Filter{ResourceAttributes{
		Name:         name,
...
func (fs FilterService) Save(f *Filter) error {
...
func (fs FilterService) Get(domainName string) (*Filter, error) {
...
func (fs FilterService) GetAll() ([]*Filter, error) {
	var epgs []*Filter
	var errors error
...
func (fs FilterService) fromJSON(data *gabs.Container) (*Filter, error) {
	var errors error
